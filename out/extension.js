...
function activate(context) {
    // ================= AUTH SCAFFOLD =================
    let disposableAuth = vscode.commands.registerCommand('extension.createReactAuth', async () => {
...
`;
        ...
        fs.writeFileSync(path.join(targetPath, 'context/AuthContext.jsx'), authContextContent.trim());
        // ====== Login.jsx ======
...
`;
        ...
        fs.writeFileSync(path.join(targetPath, 'pages/Login.jsx'), loginPageContent.trim());
        // ====== Home.jsx ======
...
export default Home;
`;
        fs.writeFileSync(path.join(targetPath, 'pages/Home.jsx'), homePageContent.trim());
        // ====== PrivateRoute.jsx ======
        const privateRoute = `
...
`;
        ...
        fs.writeFileSync(path.join(targetPath, 'components/PrivateRoute.jsx'), privateRoute.trim());
        // ====== Update App.jsx ======
        const appContent = `
...
`;
        ...
        vscode.window.showInformationMessage(`Auth scaffolding berhasil dibuat di folder: ${folderInput} 🚀`);
    });
    // ================= CRUD SCAFFOLD =================
    let disposableCrud = vscode.commands.registerCommand('extension.createCrud', async () => {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders) {
            vscode.window.showErrorMessage('Buka folder project React/Vite dulu.');
            return;
...
`;
        ...
        vscode.window.showInformationMessage(`CRUD ${crudName} berhasil dibuat di folder: ${folderInput} 🚀`);
    });
    context.subscriptions.push(disposableAuth, disposableCrud);
}
function deactivate() { }
// crudGenerator.js (bagian generate file)
function getCrudTemplate(name) {
